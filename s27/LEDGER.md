...
## S28-L41 -- VERDICT ON THE BUILT CHAIN (126/126, 18 ARMS): THE DEPLOYED READOUT AT EVERY PROJECTED J, GRAPH AND SEED IS INSIDE 0.46x MDE OF PRODUCTION AND 0.22x OF J = 0; THE TWO OTHER READOUTS ARE +0.25 TO +0.37 A WORSE THAN PRODUCTION AT EVERY J WITH THE FOLD CI ABOVE ZERO; F1 DOES NOT FIRE (THE ONE CELL THAT BEATS ITS OWN J = 0 TWIN IS A 0.28 A-WORSE READOUT CLIMBING 0.04 A, SEED-0 TWIN +0.004, PERM TWIN MOVES AS MUCH); THE BEST-OF-NINE IS THE R1 / R3 READOUT GAP, NOT A HOPPING EFFECT; THE EIGENSOLVER TIES PRODUCTION; THE REPRESENTABILITY FIT (S28-L26): THE J = 3 GAP IS BASIN SELECTION, AN OPTIMISATION SHORTFALL, NOT EXPRESSIVITY, AND THE VQE AT J = 3 IS BIMODAL (A THIRD OF THE CELLS FULLY COHERENT, THE REST AT THE J = 0 STATE); THE REGISTERED PRIOR (WORSE OR NULL) HELD; REFUTED AS REGISTERED (2026-09-19 21:02, B)

Question (`s27/PREREG_S28_B.md` F1 to F4, section 7; contract addendum 1 items 13 to 14; S28-L2 (a) to (e); S28-L22's three; S28-L26's representability fit): does the first non-diagonal Hamiltonian, consumed by any of the three readouts, emit a better BUILT CHAIN than production or than J = 0? Registered prior: WORSE or null at every J.

**Provenance.** `s28B_chain` (18 arms x 126, `s27/results/s28_B_chain_arms.txt`; per-(arm, target) checkpoints in `s27/results/s28_B_chain_rows.jsonl`) ran 2026-09-14 21:20 to 23:33 (103 targets; died with the session's low-memory kill, `RESUME_S28.md :: B addendum`); `s28B_chain_r2` resumed at target 104 on 2026-09-19 20:07 and was killed by the governor's stall breaker at 20:12 (exit 15, 1 target, box at 92 to 97% RAM from the user's programs); `s28B_chain_r3` (governor queue, priority 1) resumed at 105 at 20:22 and landed at 20:51 (exit 0, 1765 s, peak RSS 0.379 GB). No completed target was re-run (addendum 2 item 16): 2,268 rows = 126 x 18, 2,268 unique (arm, pdb), no NaN. Statistics `python s27/s28_B_analyse.py` -> `s27/results/s28_B_summary.json :: chain` (every `ST.fmt` block verbatim under `fmt`; the chain section was extended on the resume, commit 4774f4b6, with the production contrast split by FAIL18 / 108, the same-readout F1 twin, the PERM / RAND twins where projected, and the best-of-nine pricing, plus `by_readout_family`). Anchors: `rmsd_cloud` of the two J = 0 R1 arms equals S27 `vqe_rows.jsonl :: DIS` bit-for-bit on 126/126, both seeds (`anchors.s27_bit_identical_s*`; lane D's 104/104 pre-check in S28-L38 extends to 126).

**Code path (S28-L18 / S28-L27b / S28-L38).** The comparator is S27's production chain (`chain_rows.jsonl :: DIS`, mean 3.2126), a different job. To close S28-L38's caveat it was RE-PROJECTED IN THIS LANE'S OWN PROCESS on all 126 targets (`s27/s28_B_prodcheck.py`: DIS top-75 by the stable key -> `readout_uniform` -> `s24.d_harness.readout_projected`, the call `s28_B_hop.chain_main` makes; job `s28B_prodcheck`, exit 0, 597 s, peak 0.321 GB; `s27/results/s28_B_prodcheck.json`): `rmsd_chain` identical to the last digit on 126/126 (max |d| 0.0e+00 A, cloud identical on 126/126; recomputed mean 3.212625 vs stored 3.212625). Both sides of every contrast below are one code path; the branch-flip floor (mean 0.006 A, tail 0.5 A) does not enter.

**Every projected arm on the BUILT CHAIN (`summary.json :: chain`; production 3.2126; MDE = 2.8016 x SE; the decision is on the fold-clustered CI; effect = arm - comparator, negative is better):**

    arm                      chain    cloud   FAIL18  108    | vs PRODUCTION (chain): effect x/MDE  fold CI          W/L  | 108 non-FAIL18     FAIL18 (n 18)     | vs J = 0 R1, same seed      | F1: vs same readout at J = 0
    vqe|s0|NONE|J0|R1        3.2187  3.0580  6.037  2.749  | +0.0061 +0.19x [-0.021,+0.030]  61W/ 65L | +0.0041 +0.11x   +0.0177 +0.26x  | (comparator)                 | -
    vqe|s1|NONE|J0|R1        3.2241  3.0562  6.026  2.757  | +0.0115 +0.45x [-0.004,+0.032]  67W/ 59L | +0.0123 +0.41x   +0.0064 +0.25x  | (comparator)                 | -
    vqe|s0|REAL|J1|R1        3.2182  3.0541  6.020  2.751  | +0.0056 +0.21x [-0.016,+0.026]  66W/ 60L | +0.0065 +0.22x   +0.0002 +0.01x  | -0.0005 -0.03x [-0.007,+0.007] | = the previous column
    vqe|s1|REAL|J1|R1        3.2251  3.0533  6.017  2.760  | +0.0125 +0.46x [-0.005,+0.025]  67W/ 59L | +0.0151 +0.48x   -0.0029 -0.09x  | +0.0010 +0.04x [-0.020,+0.016] | = the previous column
    vqe|s0|REAL|J0.1|R1      3.2190  3.0546  6.034  2.750  | +0.0063 +0.20x [-0.019,+0.033]  62W/ 64L | +0.0050 +0.14x   +0.0143 +0.21x  | +0.0003 +0.02x [-0.005,+0.005] | = the previous column
    vqe|s0|REAL|J3|R2        3.5833  3.3813  5.554  3.255  | +0.3706 +1.58x [+0.186,+0.507]  47W/ 79L | +0.5099 +2.10x   -0.4650 -0.97x  | +0.3645 +1.54x [+0.185,+0.500] | (J = 0 R2 twin not projected)
    vqe|s0|PERM|J0.1|R1      3.2163  3.0547  6.034  2.747  | +0.0037 +0.12x [-0.019,+0.028]  62W/ 64L | +0.0018 +0.05x   +0.0148 +0.22x  | -0.0024 -0.22x [-0.008,+0.004] | = the previous column
    vqe|s0|PERM|J0.3|R3      3.4822  3.2936  5.621  3.126  | +0.2696 +1.36x [+0.113,+0.400]  48W/ 78L | +0.3809 +1.86x   -0.3985 -0.92x  | +0.2635 +1.33x [+0.106,+0.373] | -0.0090 -0.35x [-0.026,+0.010] 4/5
    vqe|s0|PERM|J1|R3        3.4624  3.2738  5.576  3.110  | +0.2498 +1.28x [+0.122,+0.368]  52W/ 74L | +0.3653 +1.82x   -0.4434 -1.02x  | +0.2437 +1.24x [+0.108,+0.360] | -0.0288 -0.56x [-0.065,+0.015] 4/5
    vqe|s0|RAND|J3|R2        3.5823  3.3817  5.571  3.251  | +0.3696 +1.58x [+0.187,+0.506]  47W/ 79L | +0.5060 +2.09x   -0.4487 -0.92x  | +0.3636 +1.55x [+0.187,+0.490] | (J = 0 R2 twin not projected)
    vqe|s1|REAL|J0.1|R3      3.4921  3.2999  5.571  3.146  | +0.2795 +1.35x [+0.133,+0.369]  47W/ 79L | +0.4009 +1.90x   -0.4490 -0.89x  | +0.2680 +1.32x [+0.122,+0.358] | -0.0369 -1.24x [-0.068,-0.011] 5/5
    vqe|s1|PERM|J0.1|R3      3.4987  3.2987  5.584  3.151  | +0.2861 +1.40x [+0.133,+0.389]  45W/ 81L | +0.4065 +1.95x   -0.4359 -0.89x  | +0.2746 +1.36x [+0.123,+0.380] | -0.0303 -0.81x [-0.052,-0.011] 5/5
    vqe|s1|PERM|J0.3|R3      3.4824  3.2944  5.589  3.131  | +0.2698 +1.34x [+0.133,+0.363]  47W/ 79L | +0.3865 +1.87x   -0.4303 -0.91x  | +0.2583 +1.30x [+0.123,+0.354] | -0.0466 -1.01x [-0.074,-0.021] 5/5
    vqe|s0|REAL|J0.1|R3      3.4949  3.3097  5.683  3.130  | +0.2823 +1.43x [+0.135,+0.419]  47W/ 79L | +0.3853 +1.86x   -0.3360 -0.77x  | +0.2762 +1.39x [+0.127,+0.391] | +0.0037 +0.20x [-0.002,+0.012] 3/5
    vqe|s0|NONE|J0|R3        3.4912  3.3096  5.672  3.128  | +0.2786 +1.42x [+0.133,+0.412]  46W/ 80L | +0.3830 +1.87x   -0.3478 -0.80x  | +0.2725 +1.38x [+0.136,+0.388] | -
    vqe|s1|NONE|J0|R3        3.5290  3.3207  5.584  3.187  | +0.3164 +1.53x [+0.173,+0.420]  41W/ 85L | +0.4418 +2.12x   -0.4359 -0.88x  | +0.3049 +1.51x [+0.163,+0.403] | -
    gs|s-1|REAL|J1|R3        3.2100  3.0597  5.971  2.750  | -0.0026 -0.09x [-0.031,+0.022]  69W/ 57L | +0.0050 +0.15x   -0.0487 -0.84x  | -0.0087 -0.30x [-0.024,+0.007] | -
    gs|s-1|REAL|J1|R2        3.2068  3.0447  5.955  2.749  | -0.0058 -0.13x [-0.026,+0.018]  73W/ 53L | +0.0039 +0.08x   -0.0641 -0.53x  | -0.0119 -0.23x [-0.053,+0.025] | -
    (W/L is printed because `ST.fmt` prints it; it is not used as evidence anywhere in this entry, S28-L22)

**F1 ("hopping helps": a VQE arm beats the SAME readout at J = 0 on the built chain beyond MDE, fold CI excluding zero, 5/5 folds, on BOTH seeds, AND beats its PERM control at the same J beyond MDE) DOES NOT FIRE.**
- The deployed readout R1: every projected J (0.1, 1), graph (REAL, PERM) and seed is within 0.0024 A of its J = 0 twin (0.02x to 0.22x MDE) and within 0.0125 A of production (0.46x at most). The realised tail is an E-prefix of m 71 to 79 at every J (S28-L21, `s28_B_mladder.json`: tail = classical top-m to 1.1e-13), so R1 could only move through m, and on the chain it does not measurably.
- R2 and R3 at every projected J, graph and seed: +0.2498 to +0.3706 A WORSE than production (1.28x to 1.58x MDE, fold CI above zero on every cell, 4/5 to 5/5 folds), as at J = 0 (R3: +0.2786 / +0.3164). The near-uniform state's most probable 75 is a dispersed set (mean pairwise RMSD 4.1 A vs 2.5 for the tail, S28-L21).
- The one cell that reads BETTER against its own J = 0 twin on the chain, `ST.fmt` verbatim:

    CHAIN F1 vqe|s1|REAL|J0.1|R3 - vqe|s1|NONE|J0|R3 (built chain)
      a 3.4921 (med 3.4089)   b 3.5290 (med 3.4998)   n=126
      effect -0.0369   median +0.0000   SE 0.0106   MDE 0.0298   effect/MDE -1.24
      iid  CI95 [-0.0585, -0.0178]
      fold CI95 [-0.0682, -0.0110]   folds same sign 5/5   per-fold 0:-0.091 1:-0.009 2:-0.055 3:-0.026 4:-0.007
      54W/35L/37T   worst degradation +0.1087 (9BAF)   p90 +0.0451   power 0.93  Type-M 1.04
      concentration: drop-top10 -0.0053 vs uniform-effect null p10/p50/p90 -0.0160/-0.0058/+0.0009 -> pctile 0.534
      VERDICT: BETTER [TYPE-M ZONE: magnitude inflated ~1.04x]

  is a readout +0.2795 A (+1.35x) ABOVE production climbing 0.0369 A toward it (S28-L2(a)'s trap, named in the prereg check); its seed-0 twin `vqe|s0|REAL|J0.1|R3` reads +0.0037 (+0.20x, 3/5): NOT replicated; its PERM control `vqe|s1|PERM|J0.1|R3` moves -0.0303 (-0.81x, fold CI [-0.0518, -0.0113]) against ITS J = 0 twin, and REAL vs PERM at that cell is -0.0067 (-0.18x): the correspondence-destroyed graph does the same (F3). The second BETTER cell is itself a control, `vqe|s1|PERM|J0.3|R3` -0.0466 (-1.01x, 5/5; +0.2698 above production). Both are the seed-1 J = 0 R3 twin being 0.04 A worse than the seed-0 one (3.5290 vs 3.4912) read from the other side. F1's both-seeds clause and its PERM clause each fail on their own.
- F3 / F4 on the chain (REAL vs PERM / RAND where the twin was projected): `vqe|s0|REAL|J0.1|R1` - PERM +0.0026 (+0.48x); `vqe|s1|REAL|J0.1|R3` - PERM -0.0067 (-0.18x); `vqe|s0|REAL|J3|R2` - RAND +0.0010 (+0.04x): silent, as on the point cloud (72 contrasts, none beyond MDE).

**S28-L22, the nine 0.7x screen cells priced as a best-of-nine (`ST.best_of_k_within` on the (126 x 9) matrix of chain differences; the n = 126 screen reproduces exactly the nine that were projected: True).** Against J = 0 R1: per-cell means +0.0003, +0.3645, -0.0024, +0.2635, +0.2437, +0.3636, +0.2680, +0.2746, +0.2583 (mean over the nine +0.2260); best cell `vqe|s0|PERM|J0.1|R1` (a CONTROL) at -0.0024 (-0.22x); per-target min-minus-mean -0.4609, across-target null -0.6385 (share 1.39), k_eff 6.12, split-half transfer -0.2277 (49% of the oracle: "residual survives: -0.2277 transfers of -0.4609 oracle"). That surviving residual is NOT a hopping effect: the nine mix three R1 cells at ~0 with six R2 / R3 cells at +0.24 to +0.36, so "choose the best cell on one half, score on the other" transfers the R1-vs-R2/R3 READOUT GAP (a WORSE effect measured at J = 0, S28-L21). Priced within each readout family (`by_readout_family`): the R1 triple has k_eff 1.34, oracle -0.0025 A per target, across-target null -0.0024 (97% accounted), split-half transfer -0.0007 A, which is 0.06x the R1 MDE; the R2 / R3 sextet: k_eff 6.27, transfer -0.0242 (11%), "NOT A SIGNAL (split-half transfers 11% of the oracle)". Against production the same: best cell `vqe|s0|PERM|J0.1|R1` at +0.0037 (+0.12x). No cell of the nine, and no order statistic over them, is a result.

**FAIL18 vs the 108 (addendum 1 item 14(a)).** R1 arms: 108 within 0.48x MDE of production, FAIL18 within 0.26x: nothing on either stratum. R2 / R3 arms: WORSE on the 108 by +0.365 to +0.510 A (1.82x to 2.12x) and on the OTHER side on FAIL18, -0.465 to -0.336 A (0.77x to 1.02x MDE at n = 18, fold CI below zero, 4/5 folds on every cell). This is S27 L9's regime read back ("every alternative helps on FAIL18 by 0.1 to 0.4 A and is null-to-harmful on the 108"): a less contracted average (the whole-pool R2, the dispersed R3) is 0.4 A closer where production fails by 6 A and 0.4 to 0.5 A further everywhere else; it is the readout, not J (the J = 0 R3 rows carry the same split: -0.348 / -0.436); the sizes at n = 18 are at or under 1x MDE (Type-M zone, sign only); and the switch is not native-free (S27 L9: routers S22 L7, S23 L7, S26 L115 null; S28-L6: no FAIL18 detector clears its permutation null). The eigensolver's FAIL18 readings (-0.049 / -0.064, 0.84x / 0.53x) are under MDE.

**The three-way split (S28-L2(b); addendum 1 item 14(c)).**
(i) HAMILTONIAN QUALITY, the exact ground state consumed by the readouts, on the chain: GS REAL J = 1 R3 (its p-top-75, Jaccard 0.960 with the DIS top-75) 3.2100, -0.0026 vs production (-0.09x, fold CI [-0.0306, +0.0223]) and -0.0087 vs J = 0 R1 (-0.30x); GS REAL J = 1 R2 (p_gs-weighted over the whole pool) 3.2068, -0.0058 vs production (-0.13x, fold CI [-0.0264, +0.0177]; its 73W/53L is not evidence, S28-L22) and -0.0119 vs J = 0 R1 (-0.23x). The hopping ground state at J = 1 re-selects the distogram's own set and emits production. F2 (VQE-R vs GS-R at the same J) is not re-tested on the chain, because no VQE R2 / R3 REAL cell at J = 1 reached the screen; the point-cloud F2 stands (the eigensolver better by 0.20 to 0.33 A beyond MDE, S28-L21), and on the chain the eigensolver's R2 / R3 tie production while every projected VQE R2 / R3 cell is 0.25 to 0.37 A worse than it.
(ii) OPTIMISATION QUALITY (`s28_B_split.json`, S28-L25; REAL, seeds 0 / 1, 126 targets): F_vqe -5.509 / -5.265 against F_gs -7.311 at J = 3 (tie at J = 1: -4.566 / -4.585 vs -4.578); the VQE beats its untrained best-of-16 on 126/126 at every J. THE REPRESENTABILITY FIT lane D asked for in S28-L26 (`s27/s28_B_represent.py`, job `s28B_represent`, exit 0, 662 s, peak 0.338 GB; `s27/results/s28_B_represent.json`; S27's 12 trainability targets, the 9-qubit endpoint register, the REAL graph, J = 3; theta fitted to maximise (g . psi(theta))^2 with g the exact ground state, Adam lr 0.15, 16 starts theta ~ N(0, 0.6^2) with draw 1 the seed-0 VQE's own theta_0, at 80 iterations (the registered loop) and 400; the re-run VQE F equals `s28_B_rows.jsonl` bit-for-bit on 24/24 (target, seed) cells: True):

    pdb    GS PR   ov(VQE s0)  ov(VQE s1)  F_vqe s0  F_vqe s1  F_gs     best ov 80  best ov 400  F at fitted 400  ov(|psi_vqe s0|) fit  coh VQE s0/s1
    1A13     283       0.794       0.787    -7.351    -7.321    -7.335      0.811       0.814        -7.362           1.000          1.000/1.000
    1I6Y     315       0.862       0.860    -7.443    -7.444    -7.286      0.874       0.877        -7.444           1.000          1.000/1.000
    1M02     329       0.000       0.850    -4.555    -7.403    -7.337      0.860       0.864        -7.416           0.986          0.000/1.000
    2BFI     277       0.021       0.000    -4.571    -4.567    -7.335      0.796       0.797        -7.311           0.895          0.022/0.000
    2LWS     310       0.000       0.000    -4.566    -4.560    -7.238      0.871       0.878        -7.435           0.979          0.000/0.000
    2MP9     290       0.000       0.000    -4.572    -4.568    -7.327      0.814       0.826        -7.405           0.971          0.000/0.000
    2P5H     298       0.002       0.009    -4.576    -4.579    -7.243      0.863       0.864        -7.467           0.979          0.006/0.003
    5Z5W     281       0.000       0.004    -4.572    -4.569    -7.345      0.796       0.796        -7.311           0.973          0.000/0.005
    6MBM     279       0.782       0.000    -7.310    -4.530    -7.332      0.796       0.798        -7.340           1.000          1.000/0.000
    7JGX     305       0.000       0.002    -4.549    -4.498    -7.324      0.846       0.848        -7.429           0.990          0.000/0.000
    8HVS     349       0.858       0.000    -7.288    -4.530    -7.300      0.872       0.880        -7.322           1.000          1.000/0.000
    9KAR     314       0.000       0.001    -4.550    -4.557    -7.327      0.853       0.855        -7.413           0.986          0.000/0.000
    median   301       0.001       0.001    -4.572    -4.567    -7.327      0.849       0.851        -7.409           0.986
    (untrained best-of-16 overlap 0.027; fitted state: hopping 0.956, coherence 1.000, PR 465; the 16 starts' finals at 400 iterations spread by at most 0.016 per target)

    Reading, in S28-L26's terms. The ground state ITSELF is not representable: the best overlap over 16 starts is 0.80 to 0.88 (median 0.851), reached from EVERY start at 80 iterations (spread <= 0.016) and unchanged at 400: an expressivity cap on the overlap, not a budget or a start effect (the 27-parameter state stays at PR ~465 against the ground state's ~301). BUT the cap does not bind on F: the fitted state has F BELOW the ground state's on 10/12 targets (median -7.409 vs -7.327; the ground state optimises <H>, alpha 1, T 0, not F) and below the VQE's reached F on 24/24 (target, seed) cells. So the circuit HOLDS a state with F 2.8 below what the optimiser reached on the failing cells, and the 1.8 gap of S28-L25 is an OPTIMISATION shortfall. Its form is BASIN SELECTION: the trained VQE state is bimodal, 7/24 cells fully sign-coherent (coherence 1.000, overlap 0.78 to 0.86 with the ground state, F -7.366 on average, at or below the fitted optimum) and 17/24 fully incoherent (coherence <= 0.02, overlap <= 0.021, F -4.557: the J = 0 value), no cell in between. The same amplitude profile with Perron-aligned signs, |psi_vqe|, IS representable (overlap 0.89 to 1.00, median 0.986) and has F -7.352: the wall between the two basins is the SIGN structure, which the gradient at J = 3 (hopping variance 820x below the diagonal terms', S28-L8b / L11) crosses from a random start on about a third of the cells and not on the rest.
    THIS CORRECTS THE READING OF S28-L8b / S28-L21 / S28-L25 ON THE FULL 126 (native-free, `s28_B_rows.jsonl`, REAL J = 3): the "circuit collects 34% / 25% of its same-sign bound" and "sign coherence 0.34 / 0.25" are MIXTURES, not partial alignments. Seed 0: 43/126 cells coherent (coherence > 0.5; 0 cells in (0.1, 0.5]) with F -7.341, hopping 0.935, m 88.7; the other 83 at F -4.560, hopping -0.001, m 74.4 (the J = 0 state). Seed 1: 32/126 coherent (0 in between), F -7.358 / -4.553. Targets coherent on both seeds 10, on either 65, on neither 61: which basin a run lands in is a property of the draw, not of the target. At J = 1 the coherent basin is found on 1 / 4 cells of 126 (seeds 0 / 1). Where the coherent basin IS found the deployed readout emits a slightly worse structure (point cloud, seed 0: R1 at J = 3 minus R1 at J = 0 is +0.0089 on the 43 coherent cells against -0.0025 on the 83 others, m 88.7 vs 74.4; R3 +0.0391 vs -0.0034): finding the hopping ground state's basin is worth nothing to the endpoint, which is the registered prior's reason (delocalising over similar candidates is the S27 consistency mechanism).
(iii) EMITTED STRUCTURE: the table. R1 emits production at every J; R2 / R3 emit 0.25 to 0.37 A worse at every J including J = 0; the eigensolver's R2 / R3 at J = 1 emit production. No leg carries a gain. Nothing quantum is claimed: the only quantum-side content is the sign structure, and it is measured as a basin the optimiser finds on a third of its draws and that does not help when found.

**S28-L2(c), the hopping value with its sign coherence beside it, every projected VQE cell (`summary.json :: arms`, n = 126, native-free; `bound` = sum |psi_i| A_ij |psi_j|; share = hop / bound coincides with coh for this near-rank-one A, a consistency check, S28-L11(3)):**

    cell               hop      bound   coh     TV vs J=0   m      PR    Jac(ptop75, DIS75)
    vqe|s0|REAL|J0.1   -0.001   0.898   0.001   0.025       74.3   428   0.177
    vqe|s1|REAL|J0.1   -0.001   0.879   0.002   0.027       71.2   414   0.180
    vqe|s0|REAL|J1     +0.007   0.893   0.009   0.098       73.7   427   0.181
    vqe|s1|REAL|J1     +0.029   0.881   0.034   0.088       71.8   415   0.177
    vqe|s0|REAL|J3     +0.318   0.913   0.339   0.155       79.3   436   0.163
    vqe|s0|PERM|J0.1   -0.001   0.896   0.001   0.026       74.4   426   0.178
    vqe|s1|PERM|J0.1   -0.000   0.877   0.002   0.027       71.2   414   0.180
    vqe|s0|PERM|J0.3   -0.001   0.893   0.001   0.050       73.9   426   0.179
    vqe|s1|PERM|J0.3   -0.000   0.879   0.002   0.046       71.3   413   0.181
    vqe|s0|PERM|J1     +0.014   0.890   0.017   0.096       73.7   428   0.180
    vqe|s0|RAND|J3     +0.323   0.912   0.342   0.163       78.7   439   0.165
    gs|s-1|REAL|J1     +0.499   0.499   1.000   0.956        5.5    62   0.960
    (J = 0: hop 0, coh 0.001, m 74.1 / 71.2; the J = 3 means are the mixtures of (ii))

**S28-L2(d).** No GS row at J = 0 was projected. Wherever GS-R3 at J = 0 appears in the point-cloud tables (S28-L21; `s28_B_rows.jsonl :: gs|s-1|NONE|J0`) it is DEGENERATE (the one-hot argmin, 74 exact-zero ties broken by the key) and is never compared; nothing here compares it.

**S28-L2(e), the best J as an order statistic.** Only the best-J rung of R1 (J = 1, chosen on the point-cloud mean and priced there) was projected, so the J grid is priced on the point cloud (`summary.json :: contrasts.GRID|*`, `ST.best_of_k_within` over the 4 non-zero rungs): REAL R1 seed 0 / 1 k_eff 3.23 / 3.48, split-half transfer +0.0030 / +0.0004 (-12% / -2% of the oracle), NOT A SIGNAL; REAL R3 k_eff 3.77 / 3.88, transfer -0.0051 / +0.0040, NOT A SIGNAL. On the chain the best-J R1 arm reads -0.0005 / +0.0010 vs J = 0 (-0.03x / +0.04x). The chain's own order statistic is the nine-cell one, priced above.

**What is new against S26 / S27 (addendum 1 item 14(b)), and what it does not change.** S27 L6 ran the genuine CVaR-VQE on 80 DIAGONAL energies x 126 x 2 seeds: the tail is a subset of the classical prefix on every cell (0 violations, residual <= 6.3e-4 A), the circuit tracks the sort; S27 L7: nothing beats DIS on the built chain among 10 finalists; S25 L17: the diagonal Hamiltonian is a fixed rank ladder up to ties, so "extra expressivity has nothing to be expressive about". This lane is the first NON-DIAGONAL H in the project (off-diagonal for J > 0; the objective is not a function of p), the first measured departure of the trained state from the J = 0 state (TV 0.03 to 0.16; and now the sign-coherent basin, a state no diagonal objective has), and the first exact-eigensolver comparator. It goes beyond S25 L17 in that H now carries target-specific structure (the pool graph) and beyond S27 L6 in that the STATE is no longer the J = 0 state while the TAIL still is an E-prefix (the tail-reading operator's property, section 5 of the prereg). None of it changes S27 L7's verdict on the built chain: the deployed readout emits production at every J, the readouts that could consume the off-diagonal structure (R2, R3) are worse than production by the same 0.25 to 0.37 A at J > 0 as at J = 0, and the exact ground state, which does consume it, re-selects the distogram's set and emits production.
