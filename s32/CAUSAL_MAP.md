...
| **SETTLED (lane V, 2000 draws; INDEPENDENTLY REPRODUCED by lane P from its own artefact)** | the aggregate is **18 circular targets**. Score vs random 128: mean **+0.1853/+0.1872**, **median −0.0371** — *the score is BETTER on 72 of 126* — with **FAIL18 = +1.4773, 114% of the total**, and the other 108 at **−0.0300, 0.30× MDE, NOT A RESULT**. **FAIL18 is defined as targets where no pool member within 1.5 Å of the pool best survives the top-75 filter**, so a filter-versus-random contrast on it is near-circular. *Off that stratum the score is marginally BETTER than random, not worse.* And the earlier arrow is fine: **universe → 500 is −0.0718 with FAIL18 contributing 0%** — retrieval is not the problem |
| **superseded reading** | the +0.4350 is **+0.2477 pure set-size order statistic** (a *random* 128 of the 500 loses that much) **+ 0.1872 the score ordering performing WORSE THAN RANDOM** (1.06× MDE, 72W/54L, Type-M inflated ~1.10×). The 128→75 arrow splits the same way: +0.0907 size, **+0.0696 ordering, also wrong-signed** | (median −0.0380; **entirely FAIL18** — −0.0296 at 0.30× on the other 108, NOT A RESULT)
...
| **the cost is a property of the OBJECT, not a constant** | real member **−0.0007 to −0.0030**; sparse combination s=10 **+0.0002**; dense prefix average **+0.1701**; production **+0.1622**. **[CORRECTED by lane R, S32-L9: the mechanism is NOT distance from the manifold.]** Production sits at `d` = **0.705** and the sparse combinations at **0.844** — *sparse is FARTHER off-manifold* — yet they pay +0.139 and +0.007, a factor of 20. What separates them is **direction**: `cos` = −0.05 for production, **+0.40** for sparse. ***The projection taxes you for leaving the manifold in a direction that has nothing to do with your error. It is not sparsity that is cheap — it is ALIGNMENT*** |
...
| **THE OFF-MANIFOLD LAW (lane R, R-13)** | production's manifold distance `d` is **rank-determined by the pool's own disagreement**: `spearman(d, top-75 mean pairwise RMSD)` = **+0.973**, R² 0.96, **partial given chain length +0.973 and given the native error +0.970**, against a within-`n` permutation null of mean +0.095 / p95 +0.35 / max +0.63. ***Both sides are native-free.*** So the off-manifold displacement **is** the averaging artefact, and `cos_align = −0.06` says it is uncorrelated with the error. The ratio `d`/spread is **not** constant (cv 0.49) — the relation is **monotone, not a proportionality** |
| **and the repair arms are dead, for a named reason** | dilating the cloud to ideal virtual-bond length is **WORSE by +1.0425 at 2.79× MDE, 5/5 folds** — registered P1.3 **FALSIFIED**. *Because the virtual bond is contracted **22.15%** while the radius of gyration is contracted only **5.40%**: the distortion is separation-dependent, and one scalar matched to one moment is wrong at the others.* The better-motivated Rg-matched dilation is also worse (+0.0622, 1.47×). **The contraction cannot be undone by a global scale** |
