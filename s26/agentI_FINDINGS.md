...
| (2) every `verify/` audit re-run, JSON beside the tracked one, diffed | DONE: 20 of 20 ran (table in E2 and L141); every audit with a comparable tracked record reproduces its science; the differences are timings, cache keys, fields added since, and two audits that now record the repair of the defect they found. Nothing tracked was overwritten (sha256 asserted per audit; deletion guards probed) | `b0912487`, `2505e575`, closing commit | `s26/i_verify_rerun.py`, `s26/results/verify/REPORT.md`, `REPORT.json`, `<audit>.rerun.json`, `verify__<name>.json`, `s26/logs/verify_*.log`, `s26/jobs_done/verify_*.json` | L141 |
...
| (6) ledger entries and this table | DONE | closing commit | this section | L98, L102, L104, L113, L127, L128, L141, closing entry |
...
- Did not run the audits' heavy tier in the order the dictionary gave: `project_exactness` was promoted ahead of the test-covered `determinism` and `hazard` audits, and `project_iters` ahead of `degeneracy`, so the most informative audits landed first; in the end all 20 ran (the coordinator extended the deadline to let `project_stability` finish).
