...
## S29-L56 -- THE DIVERGENT DIRECTION IS CLOSED NEGATIVELY WITH A MECHANISM: A CHIMERA STATE SPACE (8 RETRIEVED PARENTS x 3-mer SEGMENTS, EXACTLY ENUMERABLE AT q <= 15) IS POORER THAN THE POOL IT WAS CUT FROM (ORACLE +0.6533 A, 5/5 FOLDS) AND 87% OF RECOMBINATION'S APPARENT VALUE IS THE ORDER STATISTIC (-0.807 A vs THE PARENTS, -0.109 A vs THE MATCHED SCRAMBLED NULL); THE PRE-REGISTERED PRIMARY IS +0.4572 A WORSE THAN PRODUCTION AT 1.18x MDE SO THE GO RULE FIRES BACKWARDS; A LOCAL TRANSVERSE FIELD PASSES LANE T's TV GATE ONLY BY DRIVING THE STATE TO |+>^q, AN EIGENVECTOR OF THE MIXER, AND ITS ENTIRE ENDPOINT CHANNEL IS m; AND AT MATCHED BUDGET BEST-OF-N FROM THE UNTRAINED CIRCUIT IS INDISTINGUISHABLE FROM THE TRAINED VQE (0.07x MDE) WHILE THE OPTIMISER PLAINLY WORKS (2026-09-20 03:50, X)

QUESTION (the charter's divergent direction, H2). Is the candidate pool the wrong state space?
Specifically: is there a space in which the quantum state is a distribution over STRUCTURAL
HYPOTHESES that the pool average cannot represent, with an interacting Hamiltonian, such that
the CVaR tail ENSEMBLE (not the argmin) emits a structure and removing the quantum stage
degrades it? Pre-registered in `s29/PREREG_S29_X.md` sections 0 to 10 and addenda 1 (the
register cap, with the measured cost that forced it), 2 (the coordinator's upgrade and lane T's
S29-L15 falsifier ladder) and 3 (the BESTOFN control) -- each written before the corresponding
number existed. ANSWER: NO, on this construction, and the space is measurably POORER than the
pool it was cut from. NO ARM CLEARS ITS BAR.

THE FORMULATION. A basis state is a CHIMERA: the chain is cut into S = min(ceil(n/3), 5)
contiguous Rosetta 3-mer segments and each segment takes its backbone torsions from one of F = 8
retrieved pool members (the DIS top-8 of the shipped K = 500 pool, exact ties broken by a stable
random key). Three qubits per segment, q = 3S in {9, 12, 15}, and 2**q = 8**S EXACTLY, so the
register is padding-free and the whole space (512 to 32,768 configurations) is exactly
enumerable -- every classical control in this entry is EXACT, not sampled.
H = H_diag + H_mix, in nats. H_diag = the shipped leave-fold-out distogram's 17-bin posterior
consumed as a pair LOG SCORE on the built configuration (not its median, its mean or the L1
Bayes risk) plus the per-fold leakage-safe Ramachandran log prior as 1-body terms.
H_mix = -Gamma sum_q X_q: the transverse field, whose meaning here is a transition between
chimeras differing in one bit of one segment's fragment index. Gamma_gap is native-free (the
median |H(x) - H(x xor e_k)| over H's own top-75 and the q bits); the grid is {0, 0.5, 1, 2} x
Gamma_gap. F(theta) = CVaR_0.15(H_diag; p_theta) - T S(p_theta) - Gamma <psi|sum X_q|psi>,
T = 1 nat, exact statevector, exact parameter-shift gradient on all three terms, Adam, 80
iterations, seeds 0 and 1. Readouts: R1 uniform average of the CVaR tail SET (the DEPLOYED
operator), R2 tail-MASS-weighted (the CVaR tail ENSEMBLE, the pre-registered primary), R3 the
state's top-512 by probability; then `s12.instrument.project` to the BUILT CHAIN, the reporting
basis. Nothing was tuned on an RMSD.

WHY THIS IS NOT A RE-RUN (contract rule 10). S13 closed the per-residue TORSION-BIN lattice by
exhaustive enumeration under physics energies; S19-S21 closed the BASIN latent -- a prior FITTED
to the pool's own marginals -- by exhaustive enumeration at n = 126, where the exact argmin ties
a zero-evaluation pool (S21 L14/L17) and the latent as a SOURCE is +0.390 worse (S21 L20);
S28-L21 closed the CANDIDATE-INDEX register, where the CVaR tail is the classical top-m prefix
to 1e-13. The chimera space is none of those: built from the retrieved members themselves,
keeping each fragment's multi-residue torsion correlations, containing its eight parents, with
an ORACLE ceiling never previously measured. Lane T (S29-L15) independently derived that a LOCAL
mixer -- the only non-commuting operator class with extensive stable rank -- is meaningful only
where basis states have local structure, which is true here and false in a candidate-index
encoding, and named this formulation as the smallest qualifying one.

THE PROBE. 12 targets (S27's trainability set), `s29X_probe12e` exit 0, wall 11,117 s, PEAK RSS
0.311 GB; `s29X_bestofn` exit 0, 175 s, 0.348 GB; `s29X_probe1` (the one-target RSS probe)
exit 0, 60.1 s, 0.298 GB. 12 files x 47 arms, all parsing, all carrying both bases.
...
## S29-L57 -- LANE P, THE VERDICT ON THE PROJECTION STAGE (126/126, BUILT CHAIN): EVERY REGISTERED FALSIFIER FAILS TO FIRE AND THE HEADLINE IS NOT "BOND IS WORSE" BUT "BOND IS INDISTINGUISHABLE FROM A MATCHED RANDOM DISPLACEMENT" -- BOND - CTRL-RAND(mean of 8) IS -0.0087 A AT 0.03x MDE WITH THE FOLD CI SPANNING ZERO, WHILE BOTH ARE ABOUT +0.73 A WORSE THAN PRODUCTION; SPAN +0.1220 (1.65x, 5/5) AND ISO +0.0737 (1.25x, 4/5) ARE ALSO WORSE; AND THE MECHANISM IS CONFIRMED WHILE THE HYPOTHESIS IS REFUTED -- MAKING THE CLOUD GEOMETRICALLY CONSISTENT GENUINELY IMPROVES THE FIT (RESIDUAL 0.8135 -> 0.6191 A) AND MAKES THE STRUCTURE WORSE (2026-09-20 09:13, P)
...
factors, zero per-target information, no target receiving its own.
  ...
  P BOND - CTRL-RAND mean-of-8 (matched-magnitude derangement of g)
    a 3.9348 (med 3.5890)   b 3.9435 (med 3.5145)   n=126
    effect -0.0087   median +0.0265   SE 0.1024   MDE 0.2868   effect/MDE -0.03
    iid  CI95 [-0.2073, +0.1933]
    fold CI95 [-0.1776, +0.1027]   folds same sign 2/5   per-fold 0:+0.089 1:-0.353 2:-0.001 3:+0.156 4:+0.042
    60W/66L/0T   worst degradation +3.7668 (9BAF)   p90 +1.4001   power 0.05  Type-M 27.59
    concentration: drop-top10 +0.1555 vs uniform-effect null p10/p50/p90 +0.0203/+0.1539/+0.2917 -> pctile 0.507
    VERDICT: NOT MEASURED (|effect| 0.0087 <= its own MDE 0.2868, 0.03x)

**0.03x MDE, the fold CI spans zero, 2/5 folds same sign, 60W/66L.** Against production both are
far worse and by the same amount:

  P BOND - PROD (BUILT CHAIN)
    a 3.9348 (med 3.5890)   b 3.2126 (med 2.9661)   n=126
    effect +0.7222   median +0.2502   SE 0.1149   MDE 0.3220   effect/MDE +2.24
    iid  CI95 [+0.5019, +0.9431]
    fold CI95 [+0.5391, +0.8847]   folds same sign 5/5   per-fold 0:+0.638 1:+0.378 2:+0.810 3:+0.728 4:+0.978
    34W/92L/0T   worst degradation +5.0991 (2N9M)   p90 +2.4233   power 1.00  Type-M 1.00
    concentration: drop-top10 +0.8796 vs uniform-effect null p10/p50/p90 +0.7300/+0.8737/+1.0268 -> pctile 0.522
    VERDICT: WORSE

  P CTRL-RAND(mean8) - PROD (BUILT CHAIN)
    a 3.9435 (med 3.5145)   b 3.2126 (med 2.9661)   n=126
    effect +0.7308   median +0.7217   SE 0.0729   MDE 0.2043   effect/MDE +3.58
    iid  CI95 [+0.5873, +0.8717]
    fold CI95 [+0.5935, +0.8537]   folds same sign 5/5   per-fold 0:+0.550 1:+0.730 2:+0.811 3:+0.573 4:+0.936
    11W/115L/0T   worst degradation +2.9242 (2MQ2)   p90 +1.7291   power 1.00  Type-M 1.00
    concentration: drop-top10 +0.8805 vs uniform-effect null p10/p50/p90 +0.7903/+0.8768/+0.9669 -> pctile 0.520
    VERDICT: WORSE
...
### 2. THE OTHER ARMS
  ...
  P SPAN - PROD (BUILT CHAIN)
    a 3.3346 (med 3.0436)   b 3.2126 (med 2.9661)   n=126
    effect +0.1220   median +0.0366   SE 0.0264   MDE 0.0738   effect/MDE +1.65
    iid  CI95 [+0.0706, +0.1731]
    fold CI95 [+0.0688, +0.1650]   folds same sign 5/5   per-fold 0:+0.086 1:+0.025 2:+0.172 3:+0.167 4:+0.150
    43W/83L/0T   worst degradation +1.1698 (2N9M)   p90 +0.5357   power 1.00  Type-M 1.00
    concentration: drop-top10 +0.1634 vs uniform-effect null p10/p50/p90 +0.1293/+0.1620/+0.1959 -> pctile 0.518
    VERDICT: WORSE

  P ISO - PROD (BUILT CHAIN)
    a 3.2863 (med 3.0572)   b 3.2126 (med 2.9661)   n=126
    effect +0.0737   median +0.0211   SE 0.0210   MDE 0.0589   effect/MDE +1.25
    iid  CI95 [+0.0336, +0.1152]
    fold CI95 [+0.0246, +0.1151]   folds same sign 4/5   per-fold 0:+0.027 1:-0.008 2:+0.101 3:+0.109 4:+0.125
    54W/72L/0T   worst degradation +0.9915 (2N9M)   p90 +0.3749   power 0.94  Type-M 1.04
    concentration: drop-top10 +0.1067 vs uniform-effect null p10/p50/p90 +0.0793/+0.1056/+0.1331 -> pctile 0.518
    VERDICT: WORSE [TYPE-M ZONE: magnitude inflated ~1.04x]

  P CTRL-GLOBAL - PROD (BUILT CHAIN)
    a 3.8705 (med 3.4404)   b 3.2126 (med 2.9661)   n=126
    effect +0.6578   median +0.7058   SE 0.0668   MDE 0.1870   effect/MDE +3.52
    iid  CI95 [+0.5228, +0.7886]
    fold CI95 [+0.5226, +0.7831]   folds same sign 5/5   per-fold 0:+0.523 1:+0.613 2:+0.753 3:+0.477 4:+0.864
    21W/105L/0T   worst degradation +2.6600 (2MQ2)   p90 +1.4984   power 1.00  Type-M 1.00
    concentration: drop-top10 +0.7991 vs uniform-effect null p10/p50/p90 +0.7103/+0.7990/+0.8766 -> pctile 0.501
    VERDICT: WORSE

  P CTRL-INV - PROD (BUILT CHAIN)
    a 3.3797 (med 3.3069)   b 3.2126 (med 2.9661)   n=126
    effect +0.1671   median +0.0293   SE 0.0629   MDE 0.1763   effect/MDE +0.95
    iid  CI95 [+0.0428, +0.2911]
    fold CI95 [+0.0390, +0.2757]   folds same sign 4/5   per-fold 0:+0.333 1:+0.221 2:+0.153 3:+0.236 4:-0.054
    49W/77L/0T   worst degradation +2.7887 (8T63)   p90 +1.1520   power 0.76  Type-M 1.16
    concentration: drop-top10 +0.2877 vs uniform-effect null p10/p50/p90 +0.2091/+0.2838/+0.3596 -> pctile 0.523
    VERDICT: NOT MEASURED (|effect| 0.1671 <= its own MDE 0.1763, 0.95x)

Means (built chain): PROD **3.2126**, ISO 3.2863, SPAN 3.3346,
CTRL-INV 3.3797, CTRL-GLOBAL 3.8705, BOND 3.9348,
CTRL-RAND mean-of-8 3.9435.
...
cloud -- PROD +0.1643, BOND -0.1512, CTRL-GLOBAL -0.1143 --
...
redraw of the multi-start's branch choice:
    ...
    mean |diff| 0.0139 A   max 0.5088 A (2LNG)   above 0.02 A on 14/126   mean signed -0.0077
...
production's particular branch, and the conclusions do not move: BOND +0.7299
(+2.26x), SPAN +0.1297
(+1.77x), ISO +0.0814
(+1.40x), CTRL-INV +0.1748
(+0.99x, still NOT MEASURED). The signed FLOOR mean is
