...
| B2a | **The ansatz ladder on AMBER: `best_of_N` — no circuit at all — WINS IT.** 2,328/2,328 cells. Every trained ansatz on AMBER is worse than not running one. **χ orders nothing**: χ=16 beats χ=8 and χ=4; the best MPS arm is χ=4. | 20 | best−worst **−0.773, SE 0.176, MDE 0.493, 17W/3L** (AMBER); **NOT MEASURED** on DIST (−0.174/MDE 0.317) and LEG (−0.078/MDE 0.345) | **B8's ansatz half CLOSED, negatively.** Quantum-resource question now closed on all three Hamiltonians. |
| B2b | **Arm-set note.** B's reported 0.37 Å span is the **MPS-only** ladder; including `best_of_N` and the ring circuit gives 0.54–0.77. Excluding the ring: 0.27 / 0.28 / 0.54. | 20 | — | **Neither figure is wrong — different arm sets.** The set must be stated before two spans are compared. |
...
| C7′ | **STAGED Legacy→AMBER is WORSE than direct AMBER — F-C3 FIRES.** Survives the move-size correction (+0.229 [+0.114,+0.365]) and is not stage starvation (staged arms spent *more* evaluations: 142/172/196 vs 102). | 30 | **+0.2331 [+0.0673,+0.3950]**, 12W/18L | **REFUTED** |
| C7″ | **The CAUSAL mechanism.** A Legacy minimisation drives AMBER **~31× deeper** into the steric singularity (log₁₀E +1.4947 [+1.1437,+2.0174], worse on 25/30) and ~28× deeper in gradient norm — **because Legacy prefers 0.45 Å more compact structures, and compaction closes the contacts that ARE the singularity.** | 30 | — | **ESTABLISHED.** *Legacy is the wrong preconditioner for AMBER for exactly the reason it is Legacy.* Supersedes C7's weaker "not supported". |
| C7‴ | The lane's own registered falsifier **did not fire** — it was attached to participation ratio, which rises (+0.0132 [+0.0019,+0.0226]) but **does not beat its matched-magnitude null** (+0.0089 [−0.0039,+0.0197]). | 30 | — | **MIS-SPECIFIED REGISTRATION**, prereg left unedited. The finding rests on the endpoint and the energy mechanism, not the metric named. |
| C6′ | **A raw λ ladder never visits an intermediate Hamiltonian at all** — stronger than C6's gradient argument. Largest single-step change **0.9989, in the FIRST interval**; Hessian spectrum at λ=0.1 equals the λ=1 spectrum **to 4 s.f. on all seven metrics**, through λ=0.9. | 30 | — | **ESTABLISHED.** It is Legacy at λ=0 and AMBER everywhere else. |
| **C10** | **THE PHYSICS HALF IS WORSE THAN A MATCHED RANDOM TAIL AT n=126** — answering BRIEF §2's transfer question (does argmin transfer to a tail selector?) **YES**. random tail 3.101; Legacy 3.208 (+0.106); AMBER 3.216 (+0.115); best mixture 3.182. | **126** | **+0.080 [+0.041,+0.119]**; **42/42 arm-rows positive, 42/42 CIs excluding zero** | **ESTABLISHED — the strongest form of the sprint's central negative.** C8 gives direction at n=12 with magnitudes unmeasured; **this gives n=126 with every CI excluding zero.** |
| C11 | **The Hamiltonian-disagreement channel is DEAD** (proposed by the coordinator to this lane). | 126 | ρ = +0.046 / −0.042 / +0.058, **all p > 0.5**; a perfect ORACLE switch gains +0.173 Å while the **min-of-k selection-bias null supplies +0.175 Å of it** | **NOT DEMONSTRATED** — the entire oracle gain is selection bias. *Caveat: top-75 pool, not E7's top-512 latent window.* |
| C11a | **Why C11 dies and E7 survives — the distinction is WHAT disagrees.** Two **scorers** disagreeing about the same candidates tells you nothing: both score the same structures with the same blindness. A **PREDICTION** disagreeing with what the source can **SUPPLY** says the prediction may be wrong here — the one thing a scorer cannot represent about itself (E6). | — | — | **The null rules out the lazy reading of the positive.** |
...
| D1a | **DIRECT REPLICATION on the ORIGINAL Sprint-20 Hamiltonians** (3,840/3,840 cells; B flagged this gap itself). At matched iterations: **AMBc/spsa −0.071 [−0.253,+0.093]**, **LEG/spsa −0.006**, **LEG/adam_fd −0.007**. AMBc/spsa's trajectory: Sprint 20 −0.649 "sig" → D n=10 −0.295 → **here n=20 −0.071**, ~11% of the original. | 20 | — | **D1 = NOT SUPPORTED confirmed on the arms the claim was about.** |
| D1b | The **only** significant cell in the whole 3,840-cell sweep is `DIST/adam_fd` **+0.149 [+0.012,+0.289] — embedding WORSE** — at **unmatched** step counts (5 vs 10). | 20 | — | **Reproduces the CONFOUND, not the lever**, and points opposite to Sprint 20's claim. |
| D1c | `DIST/spsa` −0.003 [−0.052,+0.045], own **MDE 0.071** — below the programme's old 0.084 bar. | 20 | — | **The most powered null in the study.** |
| D1d | **THE ENCODING SWEEP COMPLETES: 5,760 cells.** Sprint 20's **two** distinct significant cells are **both now null** — `AMBc/spsa` −0.071 [−0.253,+0.093] and `AMBc/nelder` −0.089 [−0.223,+0.044]. **Across all 5,760 cells only two are significant, and both have the embedding WORSE** (`DIST/adam_fd` +0.149, `DIST/nelder` +0.320), both on the distogram, neither on a Hamiltonian Sprint 20 claimed. | 20 | — | **D1 CLOSED on five independent legs.** Every surviving significant effect points opposite to the original claim. |
| D1e | **Caveat on the nelder cells.** `DIST/nelder`'s +0.320 is **below its own MDE (0.372)** — Type-M zone. And **Nelder–Mead reports no iteration count in the artefact**, so **step-count matching cannot be verified on any nelder cell.** | 20 | — | The two nelder nulls are **consistent with** the verdict but **do not control for the mechanism**. `AMBc/spsa` (256 vs 256, verified) remains load-bearing. |
